...
因此我们先从前往后遍历一遍输入的链表，将结果记录在答案数组中。最后再将得到的数组逆序即可。
...
我们在初始化时，用哈希表(`unordered_map<int,int>`)记录每个值在中序遍历中的位置，这样我们在递归到每个节点时，在中序遍历中查找根节点位置的操作，只需要O(1)的时间。此时，创建每个节点需要的时间是O(1),所以总时间复杂度是O(n).
...
```java
    ...
    private static TreeNode dfs(int[] pre, int[] in, int pl, int pr, int il, int ir) {
        ...
        int k = pos.get(pre[pl]) - il; // 为了计算中序遍历左子树的节点个数
